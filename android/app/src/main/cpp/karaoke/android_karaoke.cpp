...
oboe::DataCallbackResult MicrophoneRecorder::onAudioReady(
    ...
    // Lưu dữ liệu vào vector - chỉ khi cần thiết
    if (isRecording) {
        std::lock_guard<std::mutex> lock(dataMutex);
        size_t startIndex = recordedData.size();
        size_t numSamples = numFrames * channels;
        recordedData.resize(startIndex + numSamples);
        std::copy(inputBuffer, inputBuffer + numSamples, recordedData.data() + startIndex);
    }
...
bool MicrophoneRecorder::initialize() {
    ...
    LOGD("Input stream created with sample rate: %d, channels: %d, buffer size: %d", 
...
void MicrophoneRecorder::stopRecording() {
    ...
    isRecording = false;
    LOGD("Recording stopped, total frames: %d", static_cast<int>(recordedData.size() / channels));
}
...
bool MicrophoneRecorder::saveToFile(const std::string &filePath) {
    ...
    LOGD("  - Duration: %.2f seconds", static_cast<float>(totalSamples) / (sampleRate * channels));
